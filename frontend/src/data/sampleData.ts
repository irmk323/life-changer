...
const question = (
  ...
  result: any = null, next = addDays(3), last = result ? addDays(-2) : null, keyPoints: string[] = [], modelAnswer?: string,
): any => ({ id, domain, track, title, question: title, category, priority: "P1", modelAnswer: modelAnswer || `Write a concise answer here: start with the principle, explain the trade-off, then give a production example for “${title}”.`, personalAnswer: "", notes: "", followUps: "What trade-offs would you discuss?", latestResult: result, lastPractisedAt: last, nextReviewAt: next, keyPoints });
...
export const sampleData = (): AppState => {
  const learningItems = [
    ...
    ...javaTheoryCoreQuestions.map((x, i) => question(`java-core-${i + 1}`, "JAVA_THEORY", "CORE_JAVA", x.title, x.category, i === 0 ? "PARTIAL" : null, i === 0 ? localDate() : addDays(i + 3), undefined, x.keyPoints, x.modelAnswer)),
    ...javaTheorySpringQuestions.map((x, i) => question(`java-spring-${i + 1}`, "JAVA_THEORY", "SPRING_BOOT", x.title, x.category, null, addDays(i + 3), undefined, x.keyPoints, x.modelAnswer)),
