...
| Do not backfill LearningSession in R1 | Existing Attempts have incomplete and differently timed exposure evidence. R1 keeps `learning_session.legacy_attempt_id` nullable, with `ON DELETE SET NULL`; a missing linked session is valid history rather than a data error. |
| Use one current ProblemExposure snapshot per problem | `problem_exposure.problem_id` is unique and represents only explicitly recorded future state. `content_exposure_event` is append-only history; R1 does not infer either from legacy hints, reveals, or Attempts. |
| Keep LearningMode enum codes while moving presentation policy to data | V24 adds `learning_mode_definition`; its code, display data, order, active state, version, and requirement linkage are the R1 source of truth. The enum remains only a stable compile-time code while V23's enum column is retained and deterministically linked. |
| Strengthen, rather than rewrite, R1 schema | V25 makes requirement definition links mandatory after V24 has populated them, adds definition-order and positive-order constraints, and adds timestamps to CurriculumItem. V23 remains unchanged so deployed migration history is valid. |
